...
class IVoxNode {
   public:
    ...
    bool NNPoint(const PointT& cur_pt, DistPoint& dist_point);
...
template <typename PointT, int dim>
bool IVoxNode<PointT, dim>::NNPoint(const PointT& cur_pt, DistPoint& dist_point) {
    if (points_.empty()) {
        return false;
    }
    for (const auto& pt : points_) {
        double d = distance2(pt, cur_pt);
        dist_point = (DistPoint(d, this, &pt - points_.data()));
        return true;
    }
